    ...
    var FireExplosionManager = (function () {
        function FireExplosionManager(_mouse, _scene) {
            ...
            this._mouse.OnClick.Bind(function (event) {
                var explosionId = that._explosionIds++, explosion = // Create FireExplosion at the mouse position, note this assumes that the camera has not moved.
                // If camera moves you will need to translate the mouse position to a camera relative position (function on the game camera).
                new AnimatedSprites.FireExplosion(event.Position.X, event.Position.Y, function () {
