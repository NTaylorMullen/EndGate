(function () {
    ...
    QUnit.test("Matrix2x2 divides correctly.", function () {
        var m = new eg.Matrix2x2(2, 4, 10, 20);

        QUnit.ok(!(m === m.Divide(1)), "Dividing 1 returns a new Matrix2x2");
        QUnit.ok(!(eg.Matrix2x2(.5, .25, .1, .05) === m.DivideFrom(1)), "DivideFrom 1 returns a new Matrix2x2");
        QUnit.ok(m.Equivalent(m.Divide(1)), "Dividing 1 returns the same Matrix2x2");
        QUnit.ok(new eg.Matrix2x2(.5, .25, .1, .05).Equivalent(m.DivideFrom(1)), "DivideFrom 1 returns the portion of the original Matrix2x2");
        QUnit.ok(m.Divide(m).Equivalent(new eg.Matrix2x2(1, 1, 1, 1)), "Divide the matrix by itself results in a 1 Matrix2x2.");
        QUnit.ok(m.DivideFrom(m).Equivalent(new eg.Matrix2x2(1, 1, 1, 1)), "DivideFrom the same Matrix2x2 results in a 1 Matrix2x2.");
        QUnit.ok(m.Divide(.1).Equivalent(new eg.Matrix2x2(20, 40, 100, 200)), "Divide by small floating point numbers create floating point Matrix2x2s.");
        QUnit.ok(m.DivideFrom(.1).Equivalent(new eg.Matrix2x2(.05, .025, .01, .005)), "DivideFrom small floating point numbers create same floating point Matrix2x2s as Divide.");
    });
    ...
    QUnit.test("Matrix2x2 Clone works.", function () {
        ...
        QUnit.ok(m.Equivalent(new eg.Matrix2x2(1, 2, 3, 4)), "m is still equivalent to its original pre-clone value.");
    });
    ...
    QUnit.test("Matrix2x2 Apply works.", function () {
        var m = new eg.Matrix2x2(1.11, 2.22, 3.33, 4.44);

        m.Apply(Math.round);

        QUnit.ok(m.Equivalent(new eg.Matrix2x2(1, 2, 3, 4)));

        m.Apply(function (val) { return val + 1; });

        QUnit.ok(m.Equivalent(new eg.Matrix2x2(2, 3, 4, 5)));
    });

    QUnit.test("Matrix2x2 Transform works.", function () {
        var v = new eg.Vector2d(1, 2),
            m = new eg.Matrix2x2(1, 2, 3, 4);

        QUnit.ok(m.Transform(v).Equivalent(new eg.Vector2d(5, 11)));
    });
    ...
    QUnit.test("Matrix2x2 Transpose works.", function () {
        ...
        QUnit.ok(m.Transpose().Equivalent(new eg.Matrix2x2(1, 3, 2, 4)));
    });

    QUnit.test("Matrix2x2 Inverse works.", function () {
        var m = new eg.Matrix2x2(1, 2, 3, 4);

        QUnit.ok(m.Inverse().Equivalent(new eg.Matrix2x2(-2, 1, 1.5, -.5)));
    });
